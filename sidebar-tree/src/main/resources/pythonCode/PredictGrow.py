...
def predictGrow(file_url,file_save_url):
    ...
    # 评估每个预测时间步的RMSE
    def evaluate_forecasts(series,test, forecasts, n_lag, n_seq):
        print("这里是现在的测试结构")
    ...
    for i in range(len(data)):
        ...
        # 逆转换训练数据和预测数据
        forecasts = inverse_transform(data_single, forecasts, scaler, n_test+2)
        print(forecasts)
        # 逆转换测试数据
        ...
        print(actual)
        print("真实测试值")
        actual = inverse_transform(data_single, actual, scaler, n_test+2)
        print("actual")
        print(actual)

        # 比较预测数据和测试数据，计算两者之间的损失值
        evaluate_forecasts(data_single,actual, forecasts, n_lag, n_seq)
        # 画图
    ...
    保存预测结果
    """
    with open(file_save_url,"w",newline='') as csvfile:
        writer = csv.writer(csvfile)
        #先写入columns_name
        writer.writerow(["幼苗序号","2022/5/6-9.00"])
        #写入多行用writerows
